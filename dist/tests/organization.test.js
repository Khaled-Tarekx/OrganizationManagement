...
import { Member, Organization, } from '../src/modules/organization/models.js';
import { mongoose } from '@typegoose/typegoose';
import { createTokenFromUser } from '../src/modules/auth/utilities.js';
import { AccessLevel } from '../src/modules/organization/types.js';
let app;
let accessToken;
let organizationId;
async function signUpAndSignInUser(name, email, password) {
...
beforeAll(async () => {
    ...
    try {
        ...
        accessToken = token;
        const organization = await createOrganization('New Organization', 'An organization for testing');
...
describe('Organization Endpoints', () => {
    ...
    it('should retrieve the organization', async () => {
        ...
        expect(res.body.data).toHaveProperty('description', 'A new test organization');
    ...
    it('should invite a user to the organization without permission', async () => {
        ...
        expect(res.body).toHaveProperty('message', 'you are either not an owner or not an admin');
    });
    it('should invite a user to the organization as owner', async () => {
        const invitedEmail = faker.internet.email();
        const { user, token } = await signUpAndSignInUser('Invited User', invitedEmail, 'password123');
        const organization = await createOrganization('New Org', 'Description');
        ...
        const res = await request(app)
            ...
            .send({ user_email: invitedEmail });
        expect(res.statusCode).toBe(200);
        expect(res.body).toHaveProperty('message', 'invitation has been sent successfully');
    });
    ...
    it('should prevent updating organization without permission', async () => {
        const organization = await createOrganization('Another Org', 'Testing access restrictions');
        const res = await request(app)
    ...
    it('should return 403 when inviting an existing member to the organization', async () => {
        const invitedEmail = faker.internet.email();
        const { user } = await signUpAndSignInUser('Existing Member', invitedEmail, 'password123');
        const organization = await createOrganization('Organization with Members', 'Testing duplicate invites');
    ...
    it('should return 403 for an invalid email format when inviting a user', async () => {
        const organization = await createOrganization('Invalid Email Org', 'Testing invalid email format');
        const res = await request(app)
            .post(`/api/v1/organization/${organization._id.toString()}/invite`)
            .set('Authorization', `Bearer ${accessToken}`)
